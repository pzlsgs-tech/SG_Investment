...
const { appendTradeToSheet } = require('./sheets.js');
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (body._syncTrade) {
        ...
        // AWAIT so Vercel doesn't kill the function before Sheets sync completes
        sheetsSynced = await appendTradeToSheet(trade);
        console.log('Sheets sync result:', sheetsSynced);
      }
